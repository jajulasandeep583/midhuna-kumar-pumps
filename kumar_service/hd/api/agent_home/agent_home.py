...
from kumar_service.hd.api.agent_home.utils import (
    calculate_percentage_change,
    get_default_agent_dashboard,
    get_ticket_count,
)
from kumar_service.hd.api.analytics_utils import get_avg_time_metric
from kumar_service.hd.utils import agent_only, format_time_difference
...
def get_dashboard(reset_layout: bool = False):
    ...
    for chart in layout:
        ...
        if hasattr(frappe.get_attr("kumar_service.hd.api.agent_home.agent_home"), method_name):
            method = getattr(
                frappe.get_attr("kumar_service.hd.api.agent_home.agent_home"), method_name
            )
            chart["data"] = method()
        else:
            chart["data"] = None
