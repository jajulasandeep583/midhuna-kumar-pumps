  ...
  },
  {
    ...
    onClick: () => {
      ...
      currentStep.value = {
        ...
        videoURL: "/assets/kumar_service/desk/videos/createInviteContact.mp4",
        onClick: async () => {
...
async function getFirstTicket() {
  ...
  const ticket = await call("kumar_service.hd.api.onboarding.get_first_ticket", {
    ticket: cachedTicket,
  });
  if (ticket) {
...
async function getGeneralCategory() {
  ...
  if (!generalCategory) {
    generalCategory = await call(
      "kumar_service.hd.api.onboarding.get_general_category_id"
    );
